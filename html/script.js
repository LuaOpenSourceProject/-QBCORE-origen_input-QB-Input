...
const renderTextInput = (item) => {
	...
	const isRequired = item.isRequired == 'true' ? 'required' : '';
	...
	options.forEach((option, index) => {
		div += `<label for="radio_${name}_${index}"> <input type="radio" id="radio_${name}_${index}" name="${name}" value="${
			option.value
                ...
                ${
					(item.default ? item.default == option.value : index == 0)
						? 'checked'
						: ''
				}> ${option.text}</label>`;
	});
...
const renderCheckboxInput = (item) => {
	...
	options.forEach((option, index) => {
		div += `<label for="chk_${name}_${index}">${
			option.text
		} <input type="checkbox" id="chk_${name}_${index}" name="${name}" value="${
			option.value
		}" ${option.checked ? 'checked' : ''}></label>`;
...
const renderSelectInput = (item) => {
	...
	options.forEach((option, index) => {
		...
		div += `<option value="${option.value}" ${isDefaultValue ? 'selected' : ''}>${
			option.text
		}</option>`;
		if (isDefaultValue) {
